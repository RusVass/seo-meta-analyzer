...
type AnalysisHttpError = Error & {
  statusCode: number
  details?: unknown
}

interface FetchResponse extends Awaited<ReturnType<typeof fetch>> {
  readonly ok: boolean
...
export async function analyzeUrl(targetUrl: string): Promise<SeoAnalysis> {
  try {
...
export function mapAnalysisError(error: unknown): AnalysisErrorResponse {
  if (isAnalysisHttpError(error)) {
...
function isAnalysisHttpError(error: unknown): error is AnalysisHttpError {
  return (
    typeof error === 'object' &&
    error !== null &&
    'statusCode' in error &&
    typeof (error as AnalysisHttpError).statusCode === 'number'
  )
}
