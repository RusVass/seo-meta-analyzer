...
export async function analyzeUrl(targetUrl) {
    try {
        ...
        if (!response.ok) {
            throw createHttpError(response.status, 'Unable to fetch the page', `Server responded with status ${response.status}`);
        }
        const html = await response.text();
        const finalUrl = response.url;
        return analyzeDocument(html, targetUrl, finalUrl);
    }
    catch (error) {
        if (isAnalysisHttpError(error)) {
            throw error;
        }
        const message = error instanceof Error ? error.message : 'Unknown error';
        throw createHttpError(500, 'Analysis error', message);
    }
}
export function mapAnalysisError(error) {
    if (isAnalysisHttpError(error)) {
...
function isAnalysisHttpError(error) {
    return (typeof error === 'object' &&
        error !== null &&
        'statusCode' in error &&
        typeof error.statusCode === 'number');
}
function analyzeDocument(html, requestedUrl, finalUrl) {
    const $ = load(html);
    ...
    const overallScore = Math.round(sections.reduce((sum, { score }) => sum + score, 0) / sections.length);
...
function evaluateMetaSection($, baseUrl) {
...
function evaluateOpenGraphSection($, baseUrl) {
...
function evaluateTwitterSection($, baseUrl) {
...
function summarizeSection(tags) {
...
function deriveSectionStatus(tags) {
    if (tags.some((tag) => tag.status === 'error'))
        return 'error';
    if (tags.some((tag) => tag.status === 'warning'))
        return 'warning';
    return 'ok';
}
function deriveSummaryStatus(sections) {
...
function evaluateDescription(value) {
    if (!value) {
        return createMissingTag('meta[name="description"]', 'Meta Description', true, 'Provide a meta description (70–160 characters).');
    }
    const length = value.length;
...
function evaluateKeywords(value) {
    if (!value) {
        return {
...
function evaluateCanonical(value, baseUrl) {
    if (!value) {
        return createMissingTag('link[rel="canonical"]', 'Canonical', false, 'Set a canonical URL to protect against duplicates, especially with URL parameters.');
    }
...
function evaluateOpenGraphTitle(value) {
    if (!value) {
        return createMissingTag('meta[property="og:title"]', 'OG Title', true, 'Provide og:title to craft an engaging social preview.');
    }
    const length = value.length;
...
function evaluateOpenGraphDescription(value) {
    if (!value) {
        return createMissingTag('meta[property="og:description"]', 'OG Description', true, 'Add og:description (80–200 characters) for social previews.');
    }
    const length = value.length;
...
function evaluateOpenGraphImage(value, baseUrl) {
    if (!value) {
        return createMissingTag('meta[property="og:image"]', 'OG Image', true, 'Add og:image (minimum 1200×630 px) for rich previews.');
    }
    const resolved = resolveMaybeUrl(value, baseUrl);
    if (!resolved) {
        return {
...
function evaluateOptionalUrl(tag, label, value) {
    if (!value) {
        return {
            tag,
            label,
...
function evaluateOpenGraphType(value) {
    if (!value) {
        return {
...
function evaluateTwitterCard(value) {
    if (!value) {
        return {
...
function evaluateTwitterTitle(value) {
    if (!value) {
        return createMissingTag('meta[name="twitter:title"]', 'Twitter Title', true, 'Add twitter:title — keep it short and compelling.');
    }
    const length = value.length;
    if (length > 70) {
        return {
...
function evaluateTwitterDescription(value) {
    if (!value) {
        return createMissingTag('meta[name="twitter:description"]', 'Twitter Description', true, 'Add twitter:description (up to 200 characters).');
    }
    const length = value.length;
    if (length > 200) {
        return {
...
function evaluateTwitterImage(value) {
    if (!value) {
        return {
...
function createMissingTag(tag, label, critical, recommendation) {
    return {
        tag,
        label,
...
function buildPreviews(sections, finalUrl, baseUrl) {
    ...
    const title = meta.find((tag) => tag.tag === 'title')?.value ??
        openGraph.find((tag) => tag.tag === 'meta[property="og:title"]')?.value ??
        'Preview';
    const description = meta.find((tag) => tag.tag === 'meta[name="description"]')?.value ??
        openGraph.find((tag) => tag.tag === 'meta[property="og:description"]')?.value ??
        '';
    const ogTitle = openGraph.find((tag) => tag.tag === 'meta[property="og:title"]')?.value ?? title;
    const ogDescription = openGraph.find((tag) => tag.tag === 'meta[property="og:description"]')?.value ??
        description;
    const ogImage = openGraph.find((tag) => tag.tag === 'meta[property="og:image"]')?.value ?? undefined;
    const ogUrl = openGraph.find((tag) => tag.tag === 'og:url')?.value ?? new URL(finalUrl).toString();
    const twitterTitle = twitter.find((tag) => tag.tag === 'meta[name="twitter:title"]')?.value ?? title;
    const twitterDescription = twitter.find((tag) => tag.tag === 'meta[name="twitter:description"]')?.value ??
        description;
    const twitterImage = twitter.find((tag) => tag.tag === 'meta[name="twitter:image"]')?.value ?? ogImage;
    const twitterCard = twitter.find((tag) => tag.tag === 'meta[name="twitter:card"]')?.value ??
        'summary_large_image';
    return {
        google: {
            title,
